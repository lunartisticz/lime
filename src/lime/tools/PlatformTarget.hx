...
class PlatformTarget
{
  ...
  public function execute(additionalArguments:Array<String>):Void
  {
    ...
    if (command == "clean" || (project.targetFlags.exists("clean") && (command == "update" || command == "build" || command == "test")))
    {
      logCommand("CLEAN");
      clean();
    }
  ...
  private function copyProjectAssets(outputDirectory:String, assetDirectory:String = null)
  {
    ...
    for (asset in project.assets)
    {
      ...
      {
        var localPath = asset.targetPath;
        if (StringTools.startsWith(localPath, "assets/"))
        {
          localPath = localPath.substring(7); // Removes "assets/" prefix
        }

        var path = Path.combine(outputDirectory, Path.combine("game/assets", localPath));
